...
public class Md2HtmlTest {
    private static class Element {
        ...
        public Element(final String tag, final String begin, final String end) {
            this.tag = tag;
            this.begin = begin;
            this.end = end;
        }
    ...
    protected static void addElement(final String tag, final String begin, final String end) {
        ELEMENTS.put(begin, new Element(tag, begin, end));
    }

    protected static void addElement(final String tag, final String markup) {
        addElement(tag, markup, markup);
    }

    private final Md2HtmlChecker checker = new Md2HtmlChecker("md2html.Md2Html");
    ...
    protected void mediumRandomTest(final String... markup) {
        randomTest(100, 1000, markup);
    }

    protected void hugeRandomTest(final String... markup) {
        randomTest(100, 100_000 / TestCounter.DENOMINATOR / TestCounter.DENOMINATOR, markup);
    }
    ...
    private void randomTest(final int paragraphs, final int length, final String... markup) {
    ...
    private void paragraph(final int length, final StringBuilder input, final StringBuilder output, final List<String> markup) {
        final int h = checker.getRandom().nextInt(0, 6);
        ...
        while (input.length() < length) {
            ...
            final String middle = checker.getRandom().randomString(ExtendedRandom.ENGLISH);
    ...
    private void randomSpace(final StringBuilder input, final StringBuilder output) {
        if (checker.getRandom().nextBoolean()) {
            final String space = checker.getRandom().nextBoolean() ? " " : "\n";
            input.append(space);
            output.append(space);
        }
    }
    ...
    protected void generate(final List<String> markup, final StringBuilder input, final StringBuilder output) {
        ...
        final String type = checker.getRandom().randomItem(markup);
    ...
    private void ordinary(final List<String> markup, final StringBuilder input, final StringBuilder output, final String type) {
        final Element element = ELEMENTS.get(type);

        randomSpace(input, output);
        input.append(element.begin);
        open(output, element.tag);

        word(input, output);
        generate(markup, input, output);
        word(input, output);

        input.append(element.end);
        close(output, element.tag);
        randomSpace(input, output);
    }
    ...
    protected void word(final StringBuilder input, final StringBuilder output) {
        final String word = checker.getRandom().randomString(ExtendedRandom.ENGLISH);
        input.append(word);
        output.append(word);
    }
    ...
    private void emptyLines(final StringBuilder sb) {
        while (checker.getRandom().nextBoolean()) {
            sb.append('\n');
        }
    }
    ...
    static class Md2HtmlChecker extends MainFilesChecker {
        ...
        private void test(final String input, final String output) {
            testEquals(Arrays.asList(output.split("\n")), () -> runFiles(Arrays.asList(input.split("\n"))));
        }
    }
